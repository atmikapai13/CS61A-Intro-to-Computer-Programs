...
def piggy_points(score):
    ...
    val = score**2
    min_value = val%10
    while val > 1 and val//10:
            min_value = min(val//10, val%10, min_value)
            val = val//10
    return min_value + 3
    # END PROBLEM 2
...
def more_boar(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    def first_two_digs(x):
        while x >= 100:
            x = x//10
        return x
    player_score, opponent_score = first_two_digs(player_score), first_two_digs(opponent_score)
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            score0 = score0 + take_turn(num_rolls0, score1, dice, goal)
            if not more_boar(score0, score1):
                who = next_player(who)

        elif who == 1:
            num_rolls1 = strategy1(score1, score0)
            score1 = score1 + take_turn(num_rolls1, score0, dice, goal)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        running_high_placeholder = running_high
        pts_increase = score - last_score

        if pts_increase > running_high:
            print('Player', who, 'has reached a new maximum point gain.', pts_increase, 'point(s)!')
            running_high_placeholder = pts_increase

        return announce_highest(who, score, running_high_placeholder)
    return say
