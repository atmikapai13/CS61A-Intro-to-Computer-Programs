...
def pingpong(n):
    ...
    True
    """
    def helper(end, index=1, ppn=1, dir=1):
        if index == n:
            return ppn
        if num_eights(index) > 0:
            return helper(n, index+1, ppn-dir, -dir)
        if index%8 == 0:
            return helper(n, index+1, ppn-dir, -dir)
        if n > index:
            return helper(n, index+1, ppn+dir, dir)
...
def count_coins(change):
    ...
    True
    """
    def count_helper(change, smallest_coin=1):
        if change == 0:
            return 1
        elif change < 0:
            return 0
