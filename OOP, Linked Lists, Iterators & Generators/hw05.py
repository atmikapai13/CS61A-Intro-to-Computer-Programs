class VendingMachine:
    ...
    def add_funds(self, deposit):
        if self.inventory == 0:
            return f"Inventory empty. Restocking required. Here is your ${deposit}."
        else:
            self.funds += deposit
...
def store_digits(n):
    ...
    print('DEBUG: result is', rev)

    def helper(x):
        if x < 10:
            return Link(x%10)
        return Link(x%10, store_digits(x//10))
    return helper(rev)
    ...
    if n < 10:
        return Link(n%10)
    return store_digits(n//10), Link(n%10)
    """
    x, n = n%10, n//10
    x = Link(x)
    while n > 0:
...
def path_yielder(t, value):
    ...
    """
    x =[]
    def has_path(t, value):
        if t.label != value:
            return None
        for b in t.branches:
            if has_path(b, value):
                return x.append(b.label)
        return x
    return has_path(t, value)
    """
