...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        oprtr = scheme_eval(first, env)
        validate_procedure(oprtr)
        oprnd = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(oprtr, oprnd, env)
        # END PROBLEM 4
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        forms = self.formals
        return self.env.make_child_frame(forms, args)
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        #x = do_lambda_form(expressions, env)
        formals = expressions.first.rest
        body = expressions.rest
        validate_formals(formals)
        name = LambdaProcedure(formals, body, env)
        env.define(target.first, name)
        return target.first
        # END PROBLEM 9
    else:
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 6python3 ok --submit
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        comb = bindings.first
        validate_form(comb, 2, 2)

        names = Pair(comb.first, names)
        values = Pair(scheme_eval(comb.rest.first, env), values)
